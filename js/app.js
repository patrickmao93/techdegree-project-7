...
const dailyTraffic = document
  .getElementById("chartDailyTraffic")
  .getContext("2d");
const mobileUsers = document
  .getElementById("chartMobileUsers")
  .getContext("2d");
  ...
  switch (type) {
    ...
    default:
      alert.children[0].innerHTML =
        '<span class="bold">Alert: </span>You know nothing about charts, Jon Snow.';
      color = "#dfc75f";
      break;
  }
...
if (!localStorage.sendNotification || !localStorage.profilePublic) {
...
trafficChartOptions.addEventListener("click", e => {
  ...
  if (target.className !== "chart-header__options") {
    //prevents from clicking on ul
